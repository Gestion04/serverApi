...
export const getThistory = (req, res) => {
    try{
        ...
        jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
            ...
            const data5 = await Invest.find({number:Number(decoded?.user?.number||0)})
            const user = await Users.findOne({number:Number(decoded?.user?.number||0)})
            if(!user) return res.json({message: "User not found", success: false});
            const token = jwt.sign({ user, thistory:JSON.stringify(data),receiver:JSON.stringify(data2), reference:JSON.stringify(data3), pushs: JSON.stringify(data4), invests:JSON.stringify(data5) }, process.env.JWT_SECRET, { expiresIn: '180d' });
            
            res.json({thistory:JSON.stringify(data),receiver:JSON.stringify(data2), user, token, reference:JSON.stringify(data3), pushs: JSON.stringify(data4), invests:JSON.stringify(data5), success:true})
        })
    }catch(err){
        res.json({ message: err });
    }
        ...
        if(users
            ...
            users?.forEach(async user => {
                ...
                if(timeCalculate <= 0){
                    ...
                    await Users.findOneAndUpdate({ number:Number(number)}, { $inc: { balance: +(amount*percent/100), gainBalance: +(amount*percent/100)} })
                    await Invest.findOneAndUpdate({ amount, type, number, date, day, dateEnd, gain}, { $inc: { day: +1, gain: +(amount*percent/100)} })
                    if(day === days){
                        await new Thistory({
                            type: "Invest End",
                            number: Number(number),
                        ...
                        await Invest.findOneAndRemove({ amount, type, number, date, dateEnd })
                        await Users.findOneAndUpdate({ number:Number(number)}, { $inc: { investBalance: -amount, balance: +amount} })
                        return;
                    }
                }
            })
    ...
    try{
        jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
            ...
            await new Invest({
                ...
                type: type,
                date: new Date(),
                dateEnd: new Date((new Date()/1000+(dayTime*TypeInvests[type].days))*1000),
                day: 1,
                status: true,
                gain: 0,
            }).save()
            let reference = await Reference.findOne({ number:Number(decoded?.user?.number)})
            if(reference){
                let { percent } = TypeInvests[type];
                await Users.findOneAndUpdate({ number:Number(reference.referenceNumber)}, { $inc: { balance: +(amount*percent/100) } })
                await new Thistory({
                    type: "Reference",
...
export const getInvest = async (req, res) => {
    ...
    try{
        jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
            ...
            if(!invests) return res.json({ message: 'Invest not found', success: false });
...
export const allDataAdmin = async (req, res) => {
    ...
    try{
        jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
            ...
            if(decoded?.user?.number !== 0) return res.json({ message: 'Not admin', success: false });
            const user = await Users.find({});
            const withdraw = await Withdraw.find({});
            const invest = await Invest.find({});
            const report = await Report.find({});
            const deposited = await Deposit.find({});
            res.json({ user:JSON.stringify(user), withdraw:JSON.stringify(withdraw), invest:JSON.stringify(invest), report:JSON.stringify(report), deposited:JSON.stringify(deposited), success: true });
        })
    }catch(e){
        res.json({ message: e });
    }
